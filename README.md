...
> **منصة الدفاع السيبراني السيادية للبيئات الحساسة** — ذكاء دفاعي محلي. بلا سحابة. بلا تسريب.  
> *Sovereign Defense Intelligence. On-Premise. Air-Gapped Ready.*

## هوية المشروع (للمحكّمين)

### بيان المشكلة

الجهات الدفاعية والمنشآت الحساسة تواجه معضلة: الحلول المتقدمة غالباً سحابية وتخرج بيانات حساسة عن الحدود، والحلول المحلية التقليدية تفتقر لتحليل ذكي متقدم — إما التضحية بالسيادة أو بجودة الكشف. **C.O.A** يجمع التحليل الدفاعي (وكلاء، MITRE، YARA، تقارير حوادث) في مسار **محلي/معزول** قابل للتحكم، بدلاً من مقارنته كبديل تجاري سحابي عام.

### الجمهور المستهدف

SOC في جهات دفاعية؛ منشآت حرجة (طاقة، اتصالات، مياه، نقل)؛ وحدات **Air-Gapped**؛ شركات صناعات دفاعية. في سياق الهاكاثون: **الشركة الوطنية للأنظمة الميكانيكية (SAMI)** شريك في منظومة الدفاع الوطني — يمكن تقديم المنصة كـ **جهة مستفيدة محتملة** بعد الاختبار والتكامل التنظيمي.

### الركائز الثلاث

| الركيزة | المعنى |
|---------|--------|
| **السيادة** | تحليل وتقارير وتخزين محلي؛ **Ollama** للـ LLM. **VirusTotal** اختياري للبيئات المتصلة ويُعطّل في الوضع السيادي حتى لا تخرج مؤشرات لخدمات خارجية. |
| **التخصص الدفاعي** | MITRE ATT&CK، تقارير حوادث، YARA ([COA_Project/core/rules/baseline.yar](COA_Project/core/rules/baseline.yar)) مع إمكان **توسيع القواعد** لتهديدات إقليمية؛ دعم ICS/OT ضمن **خارطة طريق** التوسع. |
| **الجاهزية التشغيلية** | تشغيل معزول عند توفير الحزم محلياً؛ **dry-run** وموافقات قبل الإجراءات؛ وثائق وواجهات عربية مع مخرجات تقنية إنجليزية في التقارير. |

**عرض جاهز للنسخ:** [COA_Project/PITCH_HACKATHON_AR.md](COA_Project/PITCH_HACKATHON_AR.md) · **شرح مدمج:** [COA_Project/شرح_فكرة_المشروع.md](COA_Project/شرح_فكرة_المشروع.md)
...
- ✅ المسار الافتراضي للتحليل والـ LLM محلي (Ollama)
- ✅ في البيئة السيادية/المعزولة: عدم تفعيل `--vt` يحافظ على عدم إرسال مؤشرات إلى خدمات خارجية
- ✅ VirusTotal (عند التفعيل) يرسل hashes فقط (ليس الملفات كاملة)
...
**C.O.A v3.0 — دفاع سيبراني سيادي للبيئات الحساسة**

مسار محلي قابل للمعزل • 4 وكلاء AI • واجهات متعددة • 53 اختبار • MITRE ATT&CK
# COA
